...
-- One row per planned review. `completed` stays false until the learner
-- has answered every question of that topic (right or wrong).
...
CREATE TABLE IF NOT EXISTS active_recall (
    ...
    CONSTRAINT active_recall_topic_fk FOREIGN KEY (topic_id)
        ...
        ON DELETE CASCADE
        ON UPDATE CASCADE,
    CONSTRAINT active_recall_subject_fk FOREIGN KEY (subject_id)
...
COMMENT ON COLUMN active_recall.completed   IS 'false = still open; true = every question of the topic was answered.';
COMMENT ON COLUMN active_recall.topic_id    IS 'FK to topics.id — the topic this review belongs to.';
COMMENT ON COLUMN active_recall.subject_id  IS 'FK to subjects.id — denormalized for subject delete/filter.';
